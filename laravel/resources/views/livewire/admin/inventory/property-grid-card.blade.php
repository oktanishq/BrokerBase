...
🆔 Property ID: {{ $property->id }}`;
...
}" class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg transition-all duration-300 group flex flex-col {{ $property->status === 'sold' ? 'opacity-90 hover:opacity-100' : '' }}">
    <!-- Property Image -->
    ...
    <div class="px-2 py-2 border-t border-gray-100 bg-gray-50/50 flex items-center justify-around">
        ...
        @if($property->status !== 'draft')
            <button @click="shareProperty()" class="flex-1 flex flex-col gap-1 py-1 justify-center items-center text-gray-400 hover:text-green-600 transition-colors group/btn" title="Copy WhatsApp share message">
